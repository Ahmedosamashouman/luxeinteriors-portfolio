...
const Testimonials = () => {
  return (
    <section className="py-24 bg-accent text-white" id="testimonials">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <motion.div
              key={index}
              className="bg-white/10 backdrop-blur-sm p-8 border border-white/20 relative h-full flex flex-col"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              ...
              <p className="font-sans text-white/90 mb-8 flex-grow">{testimonial.quote}</p>
              <div className="flex items-center mt-auto">
                ...
                <div>
                  ...
                  <p className="font-sans text-white/70 text-sm">{testimonial.title}, {testimonial.company}</p>
                </div>
              </div>
            </motion.div>
          ))}
